    ...
    <div className="mx-auto max-w-6xl space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="border-border/40 shadow-xl relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-br from-violet-600/5 to-zinc-600/5" />
          <CardHeader>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {insights.recommendations.map((rec, i) => (
              <div key={i} className={`rounded-xl border p-5 ${rec.actionable ? 'bg-zinc-500/5 border-zinc-500/20' : 'bg-muted/30 border-border/40'}`}>
                <div className="flex items-start gap-3">
                  {rec.actionable ? (
                    <Target className="h-5 w-5 text-zinc-500 mt-0.5 shrink-0" />
                  ) : (
                  ...
                  <div>
                    ...
                    {rec.actionable && (
                      <Badge variant="outline" className="mt-3 bg-zinc-500/10 text-zinc-500 border-zinc-500/20">
                        Actionable
                      </Badge>
                    )}
                  </div>
